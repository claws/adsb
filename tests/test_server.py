...
TEST_MSG = b'MSG,3,1,1,7C79B7,1,2017/03/25,10:41:45.365,2017/03/25,10:41:45.384,,2850,,,-34.84658,138.67962,,,,,,\r\n'
...
class SBSServerTestCase(asynctest.TestCase):
    ...
    async def test_server_send_message(self):
        ...
        self.assertIn(
            "Server can't send msg, no peers available", str(cm.exception))
    ...
    async def test_server_send_to_specific_peer(self):
        ...
        self.assertIn(
            "Server can't send msg to non-existant peer", str(cm.exception))
    ...
    async def test_server_broadcast(self):
        ...
        client1 = Client(host='localhost',
                         port=self.server.port,
                         on_msg_callback=mock_handler_1)
        await client1.start()

        mock_handler_2 = unittest.mock.Mock()
        client2 = Client(host='localhost',
                         port=self.server.port,
                         on_msg_callback=mock_handler_2)
        await client2.start()
